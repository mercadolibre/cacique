...
case "$1" in
  start)
        /usr/bin/memcached -P /tmp/memcached.pid -u cacique  -d -l '+CONFIG[:memcached][:ip].to_s+'
        sleep 1
  ;;
  stop)
...
#
production:
  listens_on:  "+CONFIG[:starling][:ip].to_s+":"+CONFIG[:starling][:port].to_s+"
...
# Use a different cache store in production
 config.cache_store = :mem_cache_store,"'+CONFIG[:memcached][:ip].to_s+'"
...
case "$1" in
  start)
     exec '+Dir.pwd+'/script/starling.rb -h '+CONFIG[:starling][:ip].to_s+' >  /dev/null &
  ;;
  stop)
