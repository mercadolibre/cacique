...
def retry_save(max_retries=3)
  retry_block(max_retries, "Reintento worker: ", "error, varios reintentos") do
    yield
  end
end
...
class ExecutionWorker < Workling::Base
  ...
  def run_suite(options)
    ...
    all_suite_execution[0].calculate_status #Always "Running", but if you have all the cases commented should be "Success"
  ...
  def run_executions(options)
    ...
    options[:executions].each do |@execution|
          begin
            ...
            if @execution.status != 3 and @execution.status != 5
              @execution.status = 2 
            ...
            @execution.status = 3
            @execution.error = $!.to_s
            @execution.position_error = self.parse_position_error($!, @execution.circuit.name)
            
            cache_execution.status = 3
            cache_execution.error = $!.to_s
            cache_execution.position_error = self.parse_position_error($!, @execution.circuit.name)
            
          ensure
            @execution.time_spent = Time.now - starting_time
            cache_execution.time_spent = Time.now - starting_time
            cache_suite_execution[0].time_spent += cache_execution.time_spent 
  ...
  def parse_position_error( expt, name_circuit )
	if expt.respond_to? :position_error
		return expt.position_error
	end
	  # search in backtrace any element that corresponds with running script
	  return expt.backtrace.select{ |x| x =~ Regexp.new(name_circuit) }[0]
  rescue Exception => e
	  # if not found
	  return expt.backtrace.select{ |x| x =~ Regexp.new(RAILS_ROOT) }[0]
  end
