...
(function() {
  ...
  var replacementQueryDialog = 'With: <input type="text" style="width: 10em"> <span style="color: #888">(Press [Enter], then repeat Ctrl+G to replace the occurrences)</span>';
  function replace(cm, all) {
    dialog(cm, replaceQueryDialog, "Replace:", function(query) {
      if (!query) return;
      query = parseQuery(query);
      dialog(cm, replacementQueryDialog, 'Replace with: <span style="color: #888">(Press [Enter], then repeat Ctrl+G to replace the occurrences)</span>', function(text) {
        if (all) {
          cm.operation(function() {
            for (var cursor = cm.getSearchCursor(query); cursor.findNext();) {
